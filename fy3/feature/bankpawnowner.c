...
int do_value(string arg)
{
	...
	if( !arg || !(ob = present(arg, this_player())) )
        ...
        if( ob->query("sxwuqi") )
                return notify_fail("这是神州十大兵器，小的不敢典当？？\n");
...
int do_convert(string arg)
{
	...
	if( this_player()->query_encumbrance() + to_ob->query_weight()
...
return notify_fail("你身上的" + from_ob->query("name") + "可能换出来的" + to_ob->query("name") + "可能太重了,所以掉在地上。\n");}
	return 1;
}
...
int do_pawn(string arg)
{
	...
	return 1;
        ...
        if( ob->query("sxwuqi")&&this_player()->query("combat_exp")<1000000)
                return notify_fail("这是神州十大兵器，你不够经验，小的不敢帮你典当!\n");
                    if( ob->query("sxwuqi")&&this_player()->query("sxwuqi"))
                return notify_fail("你已经典当一把神兵了，不可以再典当了\n");
	value = ob->query("value");
	if( !(value/4) ) return notify_fail("这样东西并不值很多钱。\n");
        if( mapp(pawn) && sizeof(pawn))
                {
 ...
 // to make life more realistic
	...
	me->set_pawn(p_c, term);
       ...
       if( ob->query("sxwuqi")) {me->set("sxwuqi",1);}
	me->save();

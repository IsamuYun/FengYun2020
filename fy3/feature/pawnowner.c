...
int do_value(string arg)
{
	...
	if( !value || value < 4) printf("%s并不值几文钱。\n", ob->query("name"));
                ...
                printf("%s价值%s。\n如果你要典当(pawn)，可以拿到%s。\n如果卖断(sell)，可以拿到%s，我现在身上还有%s。\n",
			ob->query("name"), value_string(value),
			value_string(value / 4), value_string(value / 5*4),value_string(this_object()->query("richness")));
...
// to make life more realistic
	...
	me->set_pawn(p_c, term);
       ...
       if( ob->query("sxwuqi")) {me->add("sxwuqi",1);}
	me->save();
...
int do_sell(string arg)
{
	...
	value = ob->query("value");
        if( !(value/4) ) return notify_fail("这样东西并不值很多钱。\n");
         // add by tiandi :not sell selfing making thing.
        if(ob->query("owner"))      return notify_fail("这东西我估不来价，你卖给其他人吧。\n");
// to make life more realistic
        if( ((int)this_object()->query("richness")) < (value/5*4))
                return notify_fail(sprintf("%s的现钱已经不够了．．．\n",this_object()->name()));
        this_object()->add("richness",-(value/5*4));
	...
	pay_player(this_player(), value /5*4);
	destruct(ob);
...
int do_redeem(string arg)
{
	...
	amount = (int) vpawn[arg];
        ...
        if( afford = affordable(me, amount) ) {
                ...
                ob = new(ob_file);
                ob->move(me);
          // by tiandi to restore self making thing.
            ob->restore();
		me->delete_pawn(arg);
                me->delete_vpawn(arg);
                me->delete_fpawn(arg);
// to make life more realistic
       ...
       if( ob->query("sxwuqi")) {me->delete("sxwuqi");}
                me->save();

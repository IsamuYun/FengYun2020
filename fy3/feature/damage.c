...
int max_food_capacity() { return 200 + query("level")*10; }

int max_water_capacity() { return 200 + query("level")*10; }


int heal_up()
{
        int update_flag, i,tili;
	mapping my;
        tili = (int)this_object()->query("lifetili");
	if( this_object()->is_fighting() ) return -1;
	update_flag = 0;
	if (this_object()->query_temp("in_dazuo")) i = 3;
	else i = 1;
	...
	if( my["water"] < 1 && userp(this_object()) ) return update_flag;
        if( my["food"] < 1 && userp(this_object()) ) return update_flag;
         ...
         // add by Tiandi 20080722
		 // 调整回血回蓝速度

         if (this_object()->query("max_lifetili") && userp(this_object())&& !this_object()->query_temp("heal_up_time") ) {
			 this_object()->set_temp("heal_up_time",10);
                  if (tili < this_object()->query("max_lifetili"))
                    {
                              tili = tili + this_object()->query("level")/5 + 1;
                              this_object()->set("lifetili",tili);
                              update_flag++;
                    }
         } 
		this_object()->add_temp("heal_up_time",-1);
	...
	my["kee"] += (i-1)*(my["con"]  + my["force"] / 10) +1+this_object()->query_temp("apply/addkee");
	...
	if( my["max_force"] && my["force"] < my["max_force"] ) {
		my["force"] += ((int)this_object()->query_skill("force", 1) / 2) * (i-1)+1+this_object()->query_temp("apply/addforce");
		if( my["force"] > my["max_force"] ) my["force"] = my["max_force"];
		update_flag++;
	}

	if( my["max_mana"] && my["mana"] < my["max_mana"] ) {
		my["mana"] += ((int)this_object()->query_skill("spells", 1) / 2) * (i-1)+1+this_object()->query_temp("apply/addmana");

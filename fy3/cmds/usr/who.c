...
mixed main(object me, string arg, int remote)
{
        ...
        if(opt_long || opt_wiz)
        {
                ...
                while( i-- ){
                        ...
                        name=interactive(list[i])?(query_idle(list[i])>120?HIG"*"NOR+HIG :" "):HIR "#"NOR+HIR;
                        if( wizardp(list[i])){
                        name = sprintf( HIY"%s%-10s(%-11s"HIY"│"NOR + HIY,name,list[i]->name(1),
        ...
        str += HIY"─────────────────────────────────────\n"NOR;
        if(opt_number)
                str = HIY"「"+ MUD_NAME+"」"NOR + WHT"目前当前共有"+chinese_number(player_cnt)+"位使用者连线中。\n";
        else if(player_cnt)
     str = sprintf("%s共有 %s 位使用者连线中，系统负担：%s\n "HIG"*"NOR" 表示发呆中，"HIR"#"NOR" 表示断线中。"NOR+HIW"■"NOR"表示男性"HIC"■"NOR"表示女性"NOR+HIY"■"NOR"表示在线巫师。\n\n", str, CHINESE_D->chinese_number(player_cnt),
        query_load_average());
        else

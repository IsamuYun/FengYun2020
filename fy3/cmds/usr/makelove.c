...
void over(object me,object obj)
{
        ...
        if(me->query("kee")<30||me->query("gin")<30||obj->query("kee")<30||obj->query("gin")<30)
        {
                if(me->query("gin")<30||me->query("kee")<30)
                {
...
过去！\n"NOR,me);
                ...
                obj->add("gin",obj->query_con()-55+random(10));
                break;
        case 1:
                message_vision(HIR "$N突然大叫一声，显然是极度兴奋，紧接着竟晕死
过去！\n"NOR,obj);
                ...
                me->add("gin",obj->query_con()-55+random(10));
                break;
        default:
                if(random(1))
                {

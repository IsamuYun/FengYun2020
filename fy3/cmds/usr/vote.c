...
int main(object me, string arg)
{
	...
	case "redis":
         if(!wizardp(me)) {
         if( CRON_D->query_last_hard_dis() > time() - 1800)
	 return notify_fail("离上此从新分布还不到半个小时！\n");
	 result = VOTE_D->add_vote(me,what,who);
         if(!result)
         return notify_fail("你或者你所在的IP已经投过票了！\n");
	...
	CRON_D->set_last_hard_dis();
	        ...
	        ips = VOTE_D->query(who+"/"+what);
		    for(i=0;i<sizeof(VOTE_D->query(who+"/"+what));i++)
		    {
		    	VOTE_D->delete_temp(ips[i]);
		    }	        	
	VOTE_D->delete(who+"/"+what);
        message("system",msg,users());  
           }                                

...
int main(object me, string arg)
{
        ...
        if (interactive(me) && stringp(no_follow = ob->query("env/no_follow")))
        {
                if ((no_follow == "all" || no_follow == "ALL" ||
                     me->query("id")==ob->query("env/no_follow")) )
                {
                        // user refuse to follow
                        return notify_fail("人家现在不想让你跟随！！\n");
                }
        }
        if( !ob->is_character() )

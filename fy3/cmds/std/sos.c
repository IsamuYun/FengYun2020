...
void done_post(object me, mapping note, string text)
{
    ...
    tell_object(me, WHT "你可以先忙你自己的事情，过一段时间后可以用sos read查看是否被解决。\n" NOR);
    save();
    return;
}
...

{
    ...
    for( i = 0; i<sizeof(notes); i++){
        str = sprintf("%d) %-50s %10s %s \n",
               i+1, notes[i]["title"], notes[i]["solve"]==""?HIR"No solved"NOR:"By "+notes[i]["solve"],
...
void do_read(object me, int i) 
{
    ...
    s = sprintf("%d) %-45s %10s %s\n"
              "==----------------------------------------------------------wlcq----==\n"
              "%s",
     i, notes[i-1]["title"], notes[i-1]["solve"]==""?HIR"目前还没有解决"NOR:"已经由"+notes[i-1]["solve"]+"解决了",
...
//    if (file_size(SOS_DIR + SOS_FILENAME) == -1) {
  //      notes = ({ });
    ...
    if (!arg) {
        if(!wiz_level(me)){
            tell_object(me, WHT "你正在" HIY MUD_NAME WHT "里辛苦地挖着泥巴，突然发现一只大个的臭虫向你爬来。\n" NOR);
            tell_object(me, WHT "你赶紧向巫师们发出紧急求救信号：ＳＯＳ！！！\n" NOR);

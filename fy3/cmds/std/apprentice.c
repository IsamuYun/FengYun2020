...
int main(object me, string arg)
{
	...
	if( me->is_apprentice_of(ob) ) {
		message_vision("$N恭恭敬敬地向$n磕头请安，叫道：「师父！」\n", me, ob);
			write("有了门派后，你就可以使用backhome这个指令瞬间回到门派附近。\n");
		ob->re_rank(me);
	...
	if( !mapp( ob->query("family") ) )
        ...
        if( userp(ob) && (int) ob->query("family/privs") != -1 )
            ...
            if(userp(ob) && (ob->query("combat_exp")/1000000) < ob->query("tudi")/2) return notify_fail(ob->name() + "的收徒名额已经超过了。\n");

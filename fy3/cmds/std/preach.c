...
int main(object me, string arg)
{
	...
	if( victim->is_fighting() ) 
        ...
        if((int)me->query_skill("daode",1) < 25 )
                return notify_fail(victim->name()+"好象不会理会你似的，可能是由于你的道德经领悟得还不够深。\n");
	...
	if ( ob->query("equipped") || ob->query("no_drop") || ob->query("owner")|| ob->query("level") )
	...
	(int)victim->query_cps() * (int)victim->query_cps() * (int)victim->query_cps()  ;

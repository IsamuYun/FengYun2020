...
if( (sscanf(file, LOG_DIR + "%*s")  || sscanf(file, "/open/%*s") || sscanf(file, DATA_DIR + "%*s")) && func=="write_file" ) return 1;
 if( sscanf(file, DATA_DIR + "%*s") && func=="save_object" ) return 1;
	// 让用户和可存储对象能存储自己的

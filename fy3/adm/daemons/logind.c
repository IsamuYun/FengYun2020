...
string *banned_name = ({
        ...
        "ÏµÍ³", "ÀîµÇ»Ô",  "ÀîºéÖ¾",  "ºËĞÄ", "ÏµÍ³ºËĞÄ","³¤ËïÎŞÇé","·ÉÌì",
});
string *banned_id= ({
...
//	shutdown(0);
	if (BAN_D->is_banned(query_ip_name(ob))  ||
	    BAN_D->is_banned(query_ip_number(ob))) {
                write("¶Ô²»Æğ£¬ÇëÔÚÏòÌìÉñ³ÂËß£¬ÎÒÃÇ»á¾¡¿ìºÍÄãÁªÏµ¡£\n");
		destruct(ob);
                return ;
        }
 ...
 if (iplimit>20)
        {
		write("¶Ô²»Æğ£¬" + MUD_NAME + "ÏŞÖÆÏàÍ¬ip¶àÖØµÇÂ¼¡£\n");
		destruct(ob);
		return;
        }
               printf(HIR"Ò»ÊÀ·çÔÆ¹²ÓĞ"NOR+HIC"%s"+NOR+HIR"¸öÍæ¼Ò×¢²áÁËµµ°¸¡£\n"NOR,chinese_number(ppl1_cnt) );
       printf(HIR"Ò»ÊÀ·çÔÆÄ¿Ç°¹²ÓĞ"+NOR+HIC+"%s"+NOR+HIR+"Î»Íæ¼ÒÓÎÏ·ÖĞ¡¢¼°"+NOR+HIC+"%s"+NOR+HIR+"Î»³¢ÊÔÁ¬ÏßÕß¡£\n"NOR,
...
ÈËÎïÍ¨ÖªÎ×Ê¦´¦Àí¡£\n");
        ...
        if(new_user_lock) {
        write("ÏÖÔÚÒ»ÊÀ·çÔÆ²»½ÓÊÜĞÂµÄÈËÎï£¬ÇëÉÔºòÔÙ´´ÔìĞÂµÄÈËÎï¡£\n");
	destruct(ob);
	return;
	}
...
user->query("mud_age"), user->query("QUEST"), ctime(time()) ) );
			...
			return;
                       ...
                       if(time()-(int)user->query("quit_time") < 5)
                        {
                        write(HIR"\nÄã¸ÕÍË³öÓÎÏ·"+ chinese_number(time()-(int)user->query("quit_time"))+"Ãë£¬ÎªÁË¼õÇáÏµÍ³¸ºµ£Çë¹ıÎåÃëÔÙÁ¬Èë£¬Ğ»Ğ»ºÏ×÷¡£^0^\n\n");
                        destruct(user);
                        destruct(ob);
                        return;
                }
...
private void confirm_id(string yn, object ob)
{
        ...
        write( @TEXT
[35m
Ò»ÊÀ·çÔÆÊÇÒ»¸öÒÔ¹ÅÁúĞ¡ËµÎª±³¾°µÄÄà°ÍÓÎÏ·£¬ÇëÄúÏë
Ò»¸öÓĞÆøÖÊ£¬ÓĞ¸öĞÔ£¬ÓÖ²»»áÌ«Ææ¹ÖµÄÖĞÎÄÃû×Ö£¬ÌØ±ğ
ÒªÌáĞÑÄúµÄÊÇ£¬ÇëÎğÊ¹ÓÃ¹ÅÁúĞ¡ËµÖĞµÄÈËÃû¡£Õâ¸öÃû×Ö
½«´ú±íÄãµÄÈËÎï£¬¶øÇÒÍùáá½«²»ÄÜÔÙ¸ü¸Ä£¬ÇëÎñ±ØÉ÷ÖØ¡£
[37m
[31m
...
private void read_msg(string arg, object ob)
{
        write(HIY "\nÏÖÔÚ¹²ÓĞ"+HIR+"/cmds/imm/checkip"->tot_usr(ob)+HIY+"Î»Íæ¼Ò´ÓÄãµÄÕ¾µã£¨"+
...
private void get_email(string email, object ob)
{
  ...
  // set the user's body after the question is answered. The following are
        ...
        ob->set("body", USER_OB);
// ·ÀÖ¹Í¬ÑùidµÄ¶àÖØµÇÂ½£¬write by tmpa 2002.1
        user = find_body(ob->query("id"));
         if (user) {
           write("ÁíÍâÒ»¸öÈË¸Õ¸Õ×¢²áÁËÕâ¸öid£¬Äú»¹ÊÇ»»Ò»¸ö°Ñ\n\n");
           destruct(ob);
           return;
         }
...
private void get_gender(string gender, object ob, object user)
{
	...
	write("
		...
		2£®²Ø×å       £¨ÄÍÁ¦ĞÍ£©
		\n");
	write("ÇëÑ¡ÔñÄãÔÚ·çÔÆ¢óÖĞµÄÃñ×å£¨0£¬1£¬2£©£º");
...
private void get_nresp(string arg, object ob, object user)
{
	    object link_ob, obj;
        if( arg=="" ) {
                write("ÇëÑ¡ÔñÄãÔÚ¿ìÀÖÔÆÖĞµÄÃñ×å£¨0£¬1£¬2£©£º");
                input_to("get_nresp", ob, user);
                return;
        }
...
user->query("id"),
	...
	user->set("national",national[(int) (arg[0]-'0')]);
		...
		return;
                ...
                write("¶Ô²»Æğ£¬ÄúÖ»ÄÜ´Ó£¨0,1,2£©ÖĞÑ¡Ôñ£º ");
                input_to("get_nresp", ob,user);
                return;
        }
...
private init_new_player(object user)
{
	...
	switch(national){
//int tol, int fle, int agi, int dur, int intt, int str, int con, int spi, int per, int cor, int cps, int kar)
		case "ºº×å":  //Æ½ºâ
		set_attribute(user,2,2,2,2,2,2,2,2,2,2,2,2);     
			break;
	        case "Ãç×å": //(ÁéÁ¦ĞÍ£©
 	              set_attribute(user,3,1,2,1,3,1,2,3,1,2,2,3);
                        break;
         	case "Âú×å": //£¨ÌåÖÊĞÍ£©
         	          set_attribute(user,2,2,1,3,1,2,3,2,1,3,3,1);
                        break;
         	case "ÃÉ¹Å×å"://Á¦Á¿
		set_attribute(user,3,3,2,2,1,3,2,2,3,1,1,1);
                        break;
         	case "°××å": //ÁéÇÉ
                set_attribute(user,3,2,3,1,1,1,2,2,2,1,3,3);
                        break;
         	case "²Ø×å"://ÄÍÁ¦
		set_attribute(user,2,3,1,3,2,2,2,1,1,3,3,1);
                        break;
			}	
	...
	user->set("env/no_pk", "YES");
	user->set("combat_exp",2000);
	user->set("max_lifetili",250);
	user->set("level",1);
	user->set("lifetili",250);	
...
"\nÓĞÄúµÄĞÅ£¡Çëµ½·çÔÆ³ÇæäÕ¾À´Ò»ÌË£®£®\n\n" + NOR);
	...
	write( HIR+"ÉÏ´ÎÁ¬ÏßµØÖ·£º" + ob->query("last_from") + "£¨"+
        ...
        switch((string) user->query("national")){
			...
			startroom = start_loc[2];	
                        break;
                case "°××å":
			startroom = start_loc[4];
                        break;
                case "²Ø×å":
			startroom = start_loc[5];	
                        break;
                        }
		}
 ...
 "Á¬Ïß½øÈëÕâ¸öÊÀ½ç¡£\n",({user}));
      ...
      write(HIC"\n    ¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô"HIR"¿ìÀÖ·çÔÆÁ¬ÏßÍ¨Öª"NOR HIC"¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô\n");
...
// user->query("id"),query_ip_name(user)) );
	...
	STATUS_D->compare_status(user);
       TOPTEN_D->topten_checkplayer(user);
        ...
        if( user->query_skill("yiqiforce") && (user->query("class")!="dongyue"))
        {
                user->delete_skill("yiqiforce");
        }
    ...
    if( sizeof(inv) ) {
     //      write ("ÎïÆ·×ÜÊı"+sizeof(inv)+"\n\n\n");
          obj=first_inventory(user);
       //     write ("Ê×¼şÎïÆ·Ãû³Æ"+obj->query("name")+"\n\n\n");
          	
          while (obj) {
      			if (obj->query("selfmaking"))
                                    {  
             //                           write(obj->query("name")+"¼ÓÔØÍê±Ï\n");
           			   obj->restore();
                    }
...
/*
	//add by tiandi to limit int with reborn times.
	reb = user->query("reborned");
	lv = user->query_skill("literate",1);
	
          if ((int)user->query("int")>30)
         user->set("int",lv/10*2);
	caizhi = user->query("int");
//	write ("ÖØÉú´ÎÊı"+reb+"\n¶ÁÊéµÈ¼¶"+lv+"\n²ÅÖÇ"+caizhi+"\n\n");
	if ( caizhi > (120 + reb*20))
		{
		if (reb<5)
			{
			write ("²ÅÖÇÉÏÏŞµ÷ÕûÍê±Ï£¬ÈôĞèÒª»ñµÃ¸ü¸ßµÄ²ÅÖÇ£¬ÇëÑ¡ÔñÖØÉú¡£\n");
			user->set("int",120+reb*20);
			}
		}
*/
}
...
"ÉÌ","Ä²","ÙÜ","¶ú","°ÛÉÍ","ÄÏ¹Ù","Ä«","¹ş","ÚÛ","Äê","°®","Ñô","Ù¡","µÚ","Îå","ÑÔ","¸£",
			});

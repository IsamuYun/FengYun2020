...
// every 45 mins, autosave every player
        if ( !((local[1])%15)) autosave();
// On average tasks get renewed every 30 mins
        if ( !random(30)) 
        TASK_D->init_dynamic_quest();
        

//How long the system will all tasks. Added by FY@SH-Morrison
//--------------------------------------------------------------------
 if ( !((local[1])%30))
      {
...
void autosave()
{
 ...
 int i;
         user = users();
        message("system", HIG " 天亮了，又开始新一天的江湖生活了。\n", users());
        for(i=0; i<sizeof(user); i++) 
        user[i]->save();
        message("system", WHT " 太阳升了起来，又要为生计四处奔波了．．． \n" NOR, users());
}
void check_dns()
{
        mapping mud_list;

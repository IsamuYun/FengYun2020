...
//by tiandi 增加一些参数，调整了计算方式
        switch(usage) {
                case SKILL_USAGE_ATTACK:
						...
						level += ob->query_cor();
						if(ob->query("is_pet")&& ob->query("title")=="野牛")  level=level*ob->query("chengzhang")/10;
						break;
                ...
                case SKILL_USAGE_DODGE:
						...
						level += ob->query_fle();
						if(ob->query("is_pet")&& ob->query("title")=="猴子")  level=level*ob->query("chengzhang")/10;
                        break;
				...
				case SKILL_USAGE_MATTACK:
						...
						level += ob->query_per();
						if(ob->query("is_pet")&& ob->query("title")=="猫咪") level=level*ob->query("chengzhang")/10;
                        break;
...
//by tiandi for little boss
		if (victim->query("issmallboss"))
		{
			...
			if (killer->query("smallboss")%20==0 )
				...
				gift->move(killer);
				tell_object(killer,NOR"由于连续完成二十次任务，你获得了奖励了"+ gift->query("name") + "的额外奖励。\n\n" +NOR);
				}
			}
			else
			{

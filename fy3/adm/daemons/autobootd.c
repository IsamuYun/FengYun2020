...
void create()
{
	...
	int t,r;
        seteuid(ROOT_UID);
        set("channel_id", "系统精灵");
        CHANNEL_D->do_channel( this_object(), "sys", MUD_NAME+"自动重起系统已经启动。\n");
#ifdef REBOOT_TIME
...
void auto_boot()
{
	...
	last=query_time();
	if(last>0 && last<10)
		message("channel:chat", HIR"【系统消息】: 为了提高本风云的速度将在"+chinese_number(last)+"分钟后将重新启动。\n"NOR,users());
...
message("channel:chat", HIR"【系统消息】"+MUD_NAME+"要重新启动了。不要紧，您的档案已经自动存盘。\n"NOR,ob[i]);
                else
message("channel:chat", HIR"【系统消息】"+MUD_NAME+"要重新启动了。您的档案已经自动存盘失败，请与巫师联系。\n"NOR,ob[i]);
        }

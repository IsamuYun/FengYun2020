...
void create()
{
	...
	set("thief", 0);
        set("chat_msg", ({
                "小芳道：你认识我老公吗？不要杀我，不然你就知道。。！\n",
                "小芳说道：你想要就说呀，你不说我怎么知道呢，你要是说的话我不会拒绝的！\n",
                (: drool :),
                (: drool :),
                (: drool :),
                (: drool :),
...
void greeting(object ob)
{
    if ( !ob || environment(ob) != environment() )
          return;
    say( "小芳终于看到一位聊友，急忙喊到：快过来和说说话！\n");
    if (!wizardp(ob))
    {
          message_vision("小芳冲着$N念叨，我老公很忙的。\n",ob,ob);
...
int do_dajia()
{
    ...
    ob = this_player();
    message_vision("小芳突然坐在地下对着$N哭了起来！\n",ob);
    message_vision("$N两手捂住双耳说道：小芳求求你别说了\n",ob);

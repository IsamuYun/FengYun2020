...
void update_age()
{
       ...
       if(query("combat_exp")> 800000 && query("class")=="official" &&query("fenglu")!=query("age")){
     tell_object(this_object(),HIG"\n由于你对朝廷忠心耿耿，朝廷决定发放你的俸禄。\n"NOR);
     tell_object(this_object(),HIG"请你去一品师爷陈果丰那里领取俸禄。\n");
           this_object()->set("fenglu",this_object()->query("age"));
            }     
          if((query("TASK")+query("bigtasks"))> 5000 && query("add_task")!= 1){
     tell_object(this_object(),HIG"\n由于你出色完成五千个使命，天机老人决定奖励你。\n"NOR);
     tell_object(this_object(),HIG"请你去他那里领取奖品。\n");
           this_object()->set("add_task",1);
            }

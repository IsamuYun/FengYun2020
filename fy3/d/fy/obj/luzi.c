...
int do_repair(string arg)
{
	...
	lv2 = (int)me->query("lifeweapon");
    	if (lv2 < 1)
      	return notify_fail("你还不会锻造之术，怎么修理东西？\n");
	...
	if(ob->query("id")!="sword" && ob->query("id")!="blade" && ob->query("id")!="ring"
	     && ob->query("id")!="staff" && ob->query("id")!="hammer")
			 return notify_fail("这里只能修理武器。\n");
	...
	if( !ob->query("selfmaking") )
		return notify_fail("你只能修理玩家制造的武器。\n");
	if (ob->query("abradability") >= 500)
		return notify_fail("这件武器还不需要马上修理吧。\n");
	score = me->query("score");
	if (score < 50)

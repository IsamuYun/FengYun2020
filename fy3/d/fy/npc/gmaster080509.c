...
int test_dart()
{
        ...
        me=this_player();
                ...
                if(this_player()->query("jiebiao") > 600)
                {
                        command("say 你的接镖历程已经非常完满了，去接其它更有挑战性的任务吧！");
                        return 1;       
                }
        if (this_player()->query("quest/yunbiao"))
   {            
                command("say 这位"+RANK_D->query_respect(this_player())+"好象有任务还没有完成！");
                        return 1;     
       } 
...
{    
        ...
        if (file_name(environment(this_player())) !="/d/fy/ghall") {
                         command("say 我不在镖局里，没有镖给你接？");
                       return 1;
                       }
        ...
        this_player()->set("quest/yunbiao",1);

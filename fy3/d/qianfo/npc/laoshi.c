...
{
  object silk;
  if(ob->query("id") == "zw-szjing")
        {
           command("smile");
           command("say 这位" + RANK_D->query_respect(who)+ "，终于这本经书还给我了，这是给你的奖励！\n");
silk = new("/questobj/big");
silk->move(who);
        tell_object(who,"老实和尚给你一个"+silk->query("name")+"。\n");
//silk->move(this_object());
//command("give jin-nang to " + who->query("id"));
who->add("bigtasks",1);
        return 1;
        }
        tell_object(who,"老实和尚说道：这不是我想要的。\n");
        return 0;
}

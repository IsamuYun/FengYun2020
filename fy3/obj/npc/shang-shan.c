...
#define SHIZHE "/data/npc/shang-shan"
...
void create()
{
        ...
        if ( !restore() ) {
                set_name("张三", ({ "shangshan shizhe", "shangshan", "shizhe" }) );
                set("long", "这位就是嫉恶如仇，侠名远播的当今武林赏善使者。\n" );
                set("title", "赏善使者" );
                set("gender", "男性" );
                set("age", 35);
...
// 昏迷处理要修改
        ...
        me->set("title", "第" + CHINESE_D->chinese_number(me->query("generation")) + "代赏善使者");
...
/*
        new_ob = new("/obj/npc/shang-shan");
        new_ob->move("/d/center/roadn1");
        destruct(me);
*/
        return 1;
}

...
#define MASTER_FER "/data/npc/master-fer"
...
void create()
{
        set_name("飞弧", ({ "master fer", "master" }));

        set("long",@LONG
此人高大威武一张清新的面孔面上带着一种强者的气色.
原来他就是大名顶顶的飞氏集团的总裁----飞弧
你一接触到他的眼神就感到阵暖流包容着你
使你有种前所没有的安全感"
LONG
        );

        set("title", HIW "飞氏集团总裁" NOR);
        set("gender", "男性" );
        set("marry","xiaoyun");
        set("faction/name","飞氏集团");
        set("faction/master","fer");
...
{
        command("smile");
        command("say 很好，有志加入飞氏集团，可以去找总裁本人。\n");
}
...
*/
        ...
        me->set_skill("english",skill_status["english"]);

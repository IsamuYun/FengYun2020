...
void create()
{
        ...
        if ( !restore() ) {
                set_name("李四", ({ "fae shizhe", "fae", "shizhe" }) );
                set("long", "这位就是嫉恶如仇，侠名远播的当今武林罚恶使者。\n" );
                set("title", "罚恶使者" );
                set("gender", "男性" );
                set("age", 35);
...
// 昏迷处理要修改
        ...
        me->set("title", "第" + CHINESE_D->chinese_number(me->query("generation")) + "代罚恶使者");
...
/*
        new_ob = new("/obj/npc/fa-e");
        new_ob->move("/d/center/roads1");
        destruct(me);
*/
        return 1;
}

...
void create()
{
        ...
        set_name(HIB"雷神"NOR, ({ "lei god", "god" }) );
        set("long", "这是来自仙界的雷神。\n");
...
void leave()
{
        object owner;
        message("vision",
                name()+HIW"不知什么时候已经消失不见了。\n" NOR, environment(),this_object() );
        if(objectp(owner=this_object()->query("possessed")))
            owner->delete_temp("guards/lei");
...
set("combat_exp", 10000000 + random(level*level*level)/20 );
        set("str", level/4);
set("force_factor", 200+random(level)/2);

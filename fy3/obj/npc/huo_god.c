...
void create()
{
        ...
        set_name(HIY"火神"NOR, ({ "huo god", "god" }) );
        set("long", "这是来自仙界的火神。\n");
...
void leave()
{
        object owner;
        message("vision",
                name()+HIY"不知什么时候已经消失不见了。\n" NOR, environment(),this_object() );
        if(objectp(owner=this_object()->query("possessed")))
            owner->delete_temp("guards/huo");
...
set("combat_exp", 10000000 + random(level*level*level)/15 );
        set("str", level/5);
set("force_factor", 200+random(level)/3);
        set_temp("apply/damage", level/2);

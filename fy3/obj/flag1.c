...
int do_mark(string arg)
{
       ...
       if(me->query("class") != "sanqing")
        ...
        message_vision("$N取出了早已准备好的$n，在地上插了一下，只见一股绿芒渗透进了地底下。\n",me,ob);
...
int do_fly(string arg)
{
		...
		if( me->query("hubiao") )
                return notify_fail("你正在护镖任务中，先去完成任务吧！\n");
        if( me->is_fighting() )
        ...
        me->start_busy(2);
        ob->set("long","一件已经被标记过的飞行符，标记地点为"+environment(me)->query("short")+"。还可以使用"+ob->query_temp("times")+"次。\n");
        message_vision("周围绿光一闪，$N骤然出现在"+environment(me)->query("short")+"。\n",me);

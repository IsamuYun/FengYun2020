...
//                return notify_fail("段氏一阳指是只有男子才能练的武功。\n");
        if( (string)me->query_skill_mapped("force")!= "duan-force")
                return notify_fail("段氏一阳指必须配合段氏心法才能练。\n");
        if((int)me->query("max_force") < 250 )
                return notify_fail("你的内力不够！\n");
        if( me->query_temp("weapon") || me->query_temp("secondary_weapon") )
                return notify_fail("练段氏一阳指必须空手。\n");
        return 1;
}
...
int effective_level() { return 15;} // 有效等级
...
{
        return CLASS_D("dali") + "/duan/" + action;
}

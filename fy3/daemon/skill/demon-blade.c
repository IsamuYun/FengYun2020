...
int valid_learn(object me)
{
	if (me->query("level") < 40)
		return notify_fail("你的等级不够，无法练如意地魔连环八刀。\n");

	 if( (int)me->query_skill("demon-force", 1) < me->query_skill("damon-blade",1) )
                return notify_fail("你的天地人魔心法火候不足，无法练如意地魔连环八刀。\n");
	return 1;
}
...
{
	return notify_fail("如意人魔连环八刀只能通过学习来提升。\n");
	return 1;
}
...
{
        return CLASS_D("mojiao") + "/demon-blade/" + action;
}
string belong()
{
	return "mojiao";

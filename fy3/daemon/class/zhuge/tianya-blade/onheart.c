...
int perform(object me, object target)
{
	...
	||	!me->is_fighting(target) )
	 ...
	 if (!couple->query_skill("fallingstar-sword"))
		    return notify_fail("你的伴侣好象不会流星蝴蝶剑。\n");
	
	 if (!couple->query_temp("weapon"))
		    return notify_fail("你的伴侣好象没有用兵器。\n");
	  if  (couple->query_temp("weapon") ->query("skill_type")!="sword")
		      return notify_fail("你的伴侣好象没有用剑。\n");
        if (couple->is_ghost())
...
void do_remote_fight(object me, object couple, object target, int n)
{
	...
	if (me->is_busy())
	 ...
	 me->add_temp("apply/damage", enhance*4);

...
void init_room(object out_room, object me)
{
       ...
       set("long", "这里是" + me->name(1) + "为大家提供的聊"
                                "天室，中间几个桌凳，上面零零散散的摆\n着茶"
                                "水和烟灰缸，地上则是一地的瓜子皮和烟头，"
                                "除此之外空空荡荡的什么也\n没有。在门口歪歪"
        ...
        CHANNEL_D->do_channel(out_room, "sys",
                              sprintf("%s(%s)在%s建立了一个聊天室。",
                                      ...
                                      out_room->query("short")));
        if (me->query_skill("idle-force", 1) >= 300)
        {
                board = new("/obj/board/chatroom_b");
...
{
 if(time()-this_player()->query("chat_eat")<60)
return notify_fail("你刚吃饱这么快又吃了？\n");
        write(HIC"你把门神剩下的饭菜一口气吃完。\n"NOR);
this_player()->set("food",this_player()->max_food_capacity());
this_player()->set("water",this_player()->max_water_capacity());
this_player()->set("chat_eat",time());
        return 1;
}
int do_invite(string arg)
{
...
int do_closechat()
{
        ...
        for (i = 0; i < sizeof(obs); i++)
        {
                ...
                if (living(obs[i]))
                        message("vision", obs[i]->name() + "从聊天室里面走了出来。\n", environment(obs[i]), ({ obs[i] }));
                else
                        message("visoin", obs[i]->name() + "被人从聊天室里面抬了出来。\n", environment(obs[i]), ({ obs[i] }));
        }
        CHANNEL_D->do_channel(this_object(), "sys",
                              sprintf("%s聊天室被%s(%s)拆除了。",
                                      me->name(1),
                                      me->name(1), me->query("id")));
...
{
        if ( dir == "out" ) me->delete_temp("chatter");
...
家进入的限制，这是相对ban -all而言的。如果一次需要邀请多个玩
